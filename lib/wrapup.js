...
var ipath = path.join(__dirname, "../includes/"),
    boil  = "(function(modules, global){" + readFile(ipath + "require.js", "utf-8") + "\n@wrup\n@named\n@nameless})({@modules}, this)",
    wrup  = "global['@wrup'] = function(id){return (id = map[id]) ? require(id) : null}",
    req   = "require('@id')",
    glob  = 'global@object["@name"] = ' + req,
...
var WrapUp = prime({
    ...
    up: function(options){
        ...
        if (options.globalize == null) options.globalize = "global"
        ...
        for (var ns in this.named){
            if (options.globalize) globals.push(replaces(glob, {
                object: (options.globalize === "global") ? '' : '["' + options.globalize + '"]',
                id: this.named[ns],
                name: ns
            }))
