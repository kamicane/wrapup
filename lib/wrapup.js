...
var browser = require('./output/browser'),
    amd     = require('./output/amd'),
    qt      = require('./output/qt'),
    graph   = require('./output/graph')
...
var WrapUp = prime({
    ...
    options: function(options){
        ...
        if (options.sourcemapIn != null) _options.sourcemapIn = options.sourcemapIn
...
*/
    ...
    browser: function(callback){
        ...
        return browser(this.go(callback))
    },

    amd: function(callback){
        if (!callback) callback = util.noop
        return amd(this.go(callback))
    },

    qt: function(callback){
        if (!callback) callback = util.noop
        return qt(this.go(callback))
    },

    graph: function(callback){
        if (!callback) callback = util.noop
        return graph(this.go(callback))
    },
    ...
    watch: function(method, callback){
        ...
        this[method](function(err){
            ...
            self._watch(function(err){
                if (err) self.emit("warn", err)
                else self.watch(method, callback)
            })
        })
        return this
    },
    ...
    _watch: function(callback){
        ...
        prime.each(this.modules, function(module, fullpath){
            fs.readFile(fullpath, "utf-8", function(err, src){
                ...
                watchers[fullpath] = fs.watch(fullpath, util.throttle(function(event){
    ...
    walkASTForRequires: function(module){
        ...
        var walker = function(ast, parent, key){
            ...
            if (ast && ast.type == "CallExpression" && ast.callee.name == "require"){
