...
var requireAST = ujs.parse(readFile(ipath + "require.js", "utf-8"), {
    filename: "includes/require.js"
})
...
cloneModuleAST = function(){
    return moduleAST.transform(new ujs.TreeTransformer(function(node, descend){
        node = node.clone()
        descend(node, this)
        return node
    }))
...
var WrapUp = prime({
    ...
    up: function(options){
        ...
        var body = requireAST.body[0].body.expression.body

        // the window["foo"] = require("x")

        for (var ns in self.named){

            // perhaps as template and just deep clone it instead of creating
            // the AST manually

            body.push(new ujs.AST_SimpleStatement({
        ...
        var properties = requireAST.body[0].body.args[0].properties

        flat.forEach(function(m){
            var ast = cloneModuleAST()
        ...
        if (options.sourcemap){
            ...
            if (options.sourcemapIn){
                smOptions.orig = readFile(optoins.sourcemapIn, "utf-8")
            }

            sourcemap = ujs.SourceMap(smOptions)
        }
