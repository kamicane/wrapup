...
var WrapUp = prime({
    ...
    findJSON: function(file){
        ...
        while (dirname = path.dirname(dirname)){
            ...
            if (path.existsSync(json)) return json
            if (dirname === "/") break
        }
        return null
    },
    ...
    up: function(options){
        ...
        var watch = function(fullpath, mod){

            mod.watched = true

            var filedata = readFile(fullpath, "utf-8")

            watchFile(fullpath, {interval: 10}, function(){
        ...
        for (var fullpath in this.modules){
            ...
            modules[path.relative(path.join(process.cwd(), "node_modules"), fullpath)] = mod.deps.map(function(dep){
                for (var fullpath in this.modules){
                    if (this.modules[fullpath].uid === dep) return path.relative(process.cwd(), fullpath)
                }
            }, this)

            if (options.watch && !mod.watched) watch(fullpath, mod)
        }
        ...
        if (options.graph != null){
            ...
            if (typeof options.graph !== "string"){
                ...
                var ext = path.extname(options.graph)
                graph.output(ext.slice(1), options.graph)
                return ""
            }
        }
